...
function removeScripts(data) {
  ...
  data.text = data.text.replace(
    /<script[^>]*>(?<script>(.(?!\/script>))*)<\/script>/gs,
    (str, sc) => {
      if (!sc) return str;
...
module.exports = class XMEN_HTML extends XMEN_XML {
  ...
  getElementsByClassName(classes, max = Infinity) {
    ...
    runTree(this.root, (node) => {
      ...
      if (
        node.$attributes.class?.length &&
        classList.every((cl) => node.$attributes.class.includes(cl))
      )
        matches.push(node);
    });
    return matches;
  }
  getElementsByTagName(tagName, max = Infinity) {
    const matches = [];
    runTree(this.root, (node) => {
      if (matches.length >= max) return true;
  ...
  getElementsById(id, max = Infinity) {
    const matches = [];
    runTree(this.root, (node) => {
      if (matches.length >= max) return true;
